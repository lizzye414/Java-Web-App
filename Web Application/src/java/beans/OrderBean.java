...
public class OrderBean
{
   ...
   {
      //"EMA Q1 (f) orderItemsGet - To be completed";
      rest.OrderItems oclient = new rest.OrderItems();
      String response = oclient.get_application_json(String.class, oucu, password);
   
      System.out.println("orderItems get Response was " + response);
      ...
      if (Utilities.statusOkay(status))
      {
         result = Utilities.prettyOrderItems(data);
      }
      else
      {
         if (jo.containsKey("message"))
   ...
   {
      //"EMA Q1(d) orderPost method in OrderBean - To be completed";
      Orders order = new Orders();
      return order.post_application_x_www_form_urlencoded(String.class, oucu, password, clientId, latitude, longitude);
   }
   ...
   {
      //"EMA Q1(e) orderItemsPost method in OrderBean - To be completed";
      rest.OrderItems orderItem = new rest.OrderItems();
      return orderItem.post_application_x_www_form_urlencoded(String.class, oucu, password, orderId, widgetId, number, pence);
   }
